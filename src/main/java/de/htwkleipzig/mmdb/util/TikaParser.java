...
public class TikaParser {
	...
	public String startTokenizing(List<String> listOfKeywords, URL fileUrl) {
		...
		List<String> listOfTokenizedStrings = tokenize(parsePdfToXml(fileUrl),
				startsWith, endsWith);

		for (String keyword : listOfKeywords) {
			extractedString = extract(listOfTokenizedStrings, keyword,
					startsWith, endsWith);
	...
	public List<String> tokenize(String text, String startsWith, String endsWith) {
		...
		String patternString = String.format("(%s).*?(%s)",
				Pattern.quote(startsWith), Pattern.quote(endsWith));
	...
	public String extract(List<String> listOfMatches, String keyword,
		...
		for (String string : listOfMatches) {
			...
			if (string.toLowerCase().contains(keyword.toLowerCase())
					&& string.length() < (keyword.length()
							+ startsWith.length() + endsWith.length() + 5)) {
				abstractString = listOfMatches.get(count + 1);
				abstractString = cleanString(abstractString, startsWith,
						endsWith);
	...
	private static String cleanString(String abstractString, String startsWith,
			String endsWith) {
		return abstractString.replace(startsWith, "").replace(endsWith, "")
				.trim();
	}
}
